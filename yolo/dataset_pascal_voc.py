...
class Dataset(object):
    ...
    def __next__(self):
        with tf.device('/cpu:0'):
            ...
            num = 0
            if self.annotation_goes < self.num_samples:
                while num < self.batch_size:
                    ...
                    annotation = self.annotations[self.annotation_goes]  
                    images, bboxeses = self.parse_annotation(annotation)

                    contains_desired_object= False
                    for bbox in bboxeses[0]:
                        if bbox[4] in self.new_classes:
                            contains_desired_object = True 

                    if self.data_aug and contains_desired_object:
                        for i in range(self.prev_aug_image, len(images)):
                            for bbox in bboxeses[i]:
                                x1,y1, x2, y2 = bbox[0],bbox[1], bbox[2], bbox[3]
                                cv2.rectangle(images[i], (x1,y1),(x2,y2), (255,0,255), 2)

                            cv2.imshow('image', images[i])
                            cv2.waitKey(0)
                            cv2.destroyAllWindows()

                            label_sbbox, label_mbbox, label_lbbox, sbboxes, mbboxes, lbboxes = self.preprocess_true_boxes(bboxeses[i])
                            batch_image[num, :, :, :] = images[i]
                            ...
                            num +=1
                            self.prev_aug_image = i+1
                            if num == self.batch_size:                            
                                if self.prev_aug_image == 5:
                                    self.annotation_goes +=1
                                    self.prev_aug_image = 0  
                                break 
                            else:
                                if self.prev_aug_image == 5:
                    ...
                    if not self.data_aug and contains_desired_object:
                        for i , image in enumerate(images):
                            ...
                            num +=1
                            self.annotation_goes +=1 
                    if not contains_desired_object:
                        self.annotation_goes+=1 
    ...
    def parse_annotation(self, annotation):
        ...
        if self.data_aug:
            image1, bboxes1 = self.random_horizontal_flip(np.copy(image), np.copy(bboxes))
            image2, bboxes2 = self.random_crop(np.copy(image), np.copy(bboxes))
            image3, bboxes3 = self.random_translate(np.copy(image), np.copy(bboxes))
            image5, bboxes5 = np.copy(image), bboxes  # BGR 
            image1, bboxes1 = image_preprocess(np.copy(image1), [self.train_input_size, self.train_input_size], np.copy(bboxes1))
            image2, bboxes2 = image_preprocess(np.copy(image2), [self.train_input_size, self.train_input_size], np.copy(bboxes2))
            image3, bboxes3 = image_preprocess(np.copy(image3), [self.train_input_size, self.train_input_size], np.copy(bboxes3))
            image5, bboxes5 = image_preprocess(np.copy(image5), [self.train_input_size, self.train_input_size], np.copy(bboxes5))


        image4, bboxes4 = cv2.cvtColor(image, cv2.COLOR_BGR2RGB), bboxes #RGB
        image4, bboxes4 = image_preprocess(np.copy(image4), [self.train_input_size, self.train_input_size], np.copy(bboxes4))

        if self.data_aug:
            images = [image1, image2, image3, image4, image5]
            bboxeses = [bboxes1, bboxes2, bboxes3, bboxes4, bboxes5]
        else:
            images = [image4]
            bboxeses = [bboxes4]

        return images, bboxeses

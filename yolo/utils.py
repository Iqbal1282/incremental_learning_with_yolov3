...
def load_yolo_weights(model, weights_file):
	tf.keras.backend.clear_session() # used to reset layer name 
...
def draw_bbox(image, bboxes, CLASSES = '',
	...
	NUM_CLASS = read_class_names(CLASSES,dot_name_file= False)
...
def image_preprocess(image, target_size, gt_boxes = None):
	...
	scale = min(iw/w, ih/h) # 416/ 800, 416/600  so 416/800*800=416, 416/800*600 =
...
def postprocess_boxes(pred_bbox, original_image, input_size,score_threshold):
	...
	pred_coor = np.concatenate([pred_xywh[:,:2]-pred_xywh[:,2:]/2.,
		pred_xywh[:,:2]+ pred_xywh[:,2:]/2.], axis = -1)
...
def detect_image(YoloV3, image_path , output_path, input_size = 416, show = False, CLASSES = '',
	score_threshold = 0.4, iou_threshold = 0.50, rectangle_colors=''):
...
def detect_realtime(YoloV3, output_path, input_size=416, show=False, CLASSES='', score_threshold=0.3, iou_threshold=0.50, rectangle_colors=''):
    times = []
...
def detect_video(YoloV3, video_path, output_path, input_size=416, show=False, CLASSES='', score_threshold=0.4, iou_threshold=0.50, rectangle_colors=''):
    times = []
